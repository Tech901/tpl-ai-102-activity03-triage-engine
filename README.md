...
3. Configure:
   ...
   - **Region**: East US *(if East US is full, use **East US 2** or **North Central US** — see warning below)*
4. Click **Create**
...
> [!WARNING]
> **Region quota is shared across the entire class.** If you get a quota or capacity error when creating your project or deploying a model, switch to a different region: **East US 2** or **North Central US** are the best alternatives. The activity works identically in any US region. Do **not** choose a region outside the US (e.g., Australia) — the added latency will slow your API calls.
...
4. Click **Deploy** > **Customize** (not "Default settings")
5. Set **Tokens per Minute Rate Limit** to **50K** (the minimum). The full activity uses ~50K tokens total, so a small rate limit is fine — it just means requests are throttled if you exceed the limit within a single minute.
...
> [!IMPORTANT]
> **Why 50K TPM instead of the default?** The class shares a subscription-wide quota for GPT-4o. The default deployment grabs 225K TPM — with a 450K total limit, that means only 2 students can deploy per region before everyone else gets blocked. Setting 50K TPM leaves room for your classmates. If you hit rate-limit errors during the parameter sweep (Step 6), the retry logic in your code handles it automatically.
...
**1.1 Initialize the Client** — `_get_client()` and the SDK imports are pre-implemented at the top of `app/main.py`. You don't need to write client init code; `_get_client()` lazily constructs a `ChatCompletionsClient` and uses `app/_azure_endpoint.py::inference_endpoint()` to normalize the endpoint URL (the `azure-ai-inference` SDK does not auto-route bare resource endpoints to deployments, so the helper appends `/openai/deployments/<deployment>` when needed).
...
> [!IMPORTANT]
> The `azure.ai.inference` SDK takes `response_format` as a **string** (`"json_object"`), not the dict form `{"type": "json_object"}` you used in Activity 1. Activity 1 used the `openai.AzureOpenAI` SDK; here we're using `azure.ai.inference.ChatCompletionsClient`, and passing a dict will raise `ValueError: Unsupported response_format` at runtime.
